...
import { collectApprovalLiveFacts, disposeBackedTargetExecutor, executeApprovalBundle } from "../src/ui-execute.js";
...
function repoFixture(): { home: string; ledger: string; subject: string } {
  ...
  return { home, ledger, subject };
}

function plannedTarget(ledger: string, subject: string, planId: string): UiApprovalTarget {
  return target("shf_backup", ledger, subject, {
    planId,
    actionType: "trash-resolve",
    planEntryDigest: disposePlanEntryDigest(readDisposePlanEntry(ledger, planId))
  });
}

function recordById(ledger: string, id: string) {
  return readLedger(ledger).find((entry) => entry.id === id);
}
...
test("collectApprovalLiveFacts marks a selected plan without a digest stale when the reviewed plan is missing", () => {
  const { home, ledger, subject } = repoFixture();
  const approved = target("shf_backup", ledger, subject, { planId: "dispose_20260301_000000_missing" });
  ...
  const result = executeApprovalBundle(snapshot, collectApprovalLiveFacts(snapshot), () => {
    throw new Error("missing reviewed plan should not execute");
  });
...
test("collectApprovalLiveFacts lets trash-resolve resume after the subject reached plan trash", () => {
  ...
  const approved = plannedTarget(ledger, subject, plan.planId);
  const trashTarget = plan.entry?.targetPath as string;
  mkdirSync(dirname(trashTarget), { recursive: true });
  renameSync(subject, trashTarget);
  const snapshot = bundle(home, [approved], [approved.targetId]);

  const result = executeApprovalBundle(snapshot, collectApprovalLiveFacts(snapshot), disposeBackedTargetExecutor);
