...
For dispose-backed targets, approval binds to the reviewed dispose-plan entry contents, including reason, subject snapshot, target path, and retention, so replacing a same-id plan artifact after approval makes the bundle stale instead of changing what executes.
