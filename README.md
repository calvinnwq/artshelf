...
On the served page the dashboard lets a reviewer queue recommended lane approvals and submit the selected work as pending session events, while the detail drawer adds record-level forms for inspect, comment, keep/trash/resolve/defer, and dry-run requests.
