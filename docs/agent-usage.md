...
`artshelf ui review [--scope user|repo] [--port <port>] [--poll-interval-ms <ms>] [--json]` is the managed live workflow: it starts or resumes the session, serves the token-protected browser UI, keeps the poll loop attached, marks browser submissions `in_progress`, replies outcomes into activity, turns exact keep/trash/resolve/defer decisions and exact dispose dry-run requests into reviewed dispose dry-run plans (plan id plus exact approval text in the reply; defer/snooze uses a default `7d` horizon), handles dashboard lane dry-runs by preparing cleanup plans, source/missing-file checks, or purge review workbench handoffs, executes approved non-purge bundles through the existing exact-target `ui execute` core, reserves purge bundles for a separate explicit one-way-door execute, and ends the session plus server after browser close, cancelling still-pending events visibly.
...
Dashboard dry-run requests enter the agent queue as lane events: cleanup prepares reviewed cleanup plan artifacts from the rendered cleanup row snapshot, resolve checks missing files, purge-candidates prepares a delete-review workbench handoff, and registry/reconcile checks source problems.
...
The served bundle workbench at `GET /bundle/<bundle-id>` shows the selected exact targets, reviewed-only rows, and exact action from an immutable workbench source or submitted approval snapshot.
With the active token, its scriptless form can submit a revised non-empty subset through `POST /approve`, creating a new immutable submitted approval snapshot and pending approval event without editing the source snapshot or executing a workflow.
That submit carries only the source snapshot id and selected target ids; the server rehydrates the action, reviewed facts, and exact target rows from the stored source snapshot instead of trusting hidden browser target JSON.
...
The browser records exact-target triage intents and approval bundle submissions. In the managed workflow, `artshelf ui review` owns polling, replies, reviewed dry-run plan preparation for exact decision/dry-run intents, reviewed cleanup plan and purge workbench preparation for supported lane dry-runs, exact approved non-purge bundle execution through the existing `ui execute` core, purge-bundle reservation for a separate explicit one-way-door execute, and close teardown that cancels still-pending events. In manual/hosted workflows, the agent polls with `artshelf ui poll <session-id> --json`, uses `artshelf ui execute` for approved bundles or runs existing approval-gated commands only after exact human approval, replies with `artshelf ui reply`, and closes with `artshelf ui end`.
The browser captures triage intents and approval bundles only and never mutates ledgers, files, trash, or plans directly.
`artshelf ui bundle <session-id> [<bundle-id>] --json` is the agent's read surface over submitted approval bundles: with a bundle id it loads one immutable event-backed snapshot plus its resolved deliberate selection so the agent can revalidate live state before execution; with no bundle id it lists only the session's approved bundles with matching `approval_bundle_submitted` events.
Browser-only workbench source snapshots remain openable from the token-protected workbench route, but the agent-facing CLI does not list or load them.
It never executes a bundle.
`artshelf ui execute <session-id> <bundle-id> --json` is the agent's mutating path and the one `ui` subcommand that changes live state: it loads the immutable submitted approval snapshot, re-reads live ledger/registry/trash state, then runs a revalidate -> execute -> verify loop through the existing approval-gated dispose or one-way-door purge paths and replies per-target receipts plus the aggregate result to the session.
...
Treat the session token printed by `artshelf ui`, `artshelf ui serve`, and `artshelf ui review` as a secret same-machine browser capability; ending the session revokes future browser writes and served dashboard/detail/bundle access while keeping the audit trail readable.

### Managed UI review workflow

The user-facing review should behave like one attached workflow, not a manual
handoff between browser and shell commands. Prefer `artshelf ui review` for this
managed lifecycle. If a host composes the lower-level primitives itself, it
should:
