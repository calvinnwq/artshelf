...
The dashboard lets a reviewer queue recommended lane approvals and submit the selected work as pending events in the durable session log, while the detail drawer adds record-level forms for inspect, comment, keep/trash/resolve/defer, and dry-run requests.
