...
`artshelf ui review [--scope user|repo] [--port <port>] [--poll-interval-ms <ms>] [--json]` is the managed live workflow: it starts or resumes the session, serves the token-protected browser UI, keeps the poll loop attached, marks browser submissions `in_progress`, replies outcomes into activity, turns exact decision and record-level dry-run intents into reviewed dispose dry-run plans (plan id plus exact approval text in the reply; defer/snooze uses a default `7d` horizon), and ends the session plus server after browser close, cancelling still-pending events visibly.
